...
// Global listener for password/API key visibility toggle buttons
...
function wireMoreSheet(root) {
  ...
  // The More button lives in tabbar-mount (or siderail), not more-mount — search whole document.
  const opener = document.querySelector('[data-shell-more]');
...
export function initShell() {
  ...
  if (activeId) {
    ...
    const path = (window.location.pathname || '').toLowerCase();
    const file = path.split('/').pop() || 'page';
